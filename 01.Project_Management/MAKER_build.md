...
2. In the web UI, send: `@goal Create a file named maker_smoke.txt with the word ok`.
3. Observe iteration count, cost, and final state in the response.
4. Verify the file exists on disk at the uvicorn cwd (the repo root). Per-session workspaces are not yet a MAKER concept — the executor inherits uvicorn's cwd.
...
| 2 | `Test-Path .\maker_smoke.txt` | `True` (manual smoke artefact) |
| 3 | `Get-Content .\maker_smoke.txt` | `ok` |
