...
| M1 | `maker/` package skeleton | Haiku | W1 | M0 | done |
| M2 | State dataclasses (`state.py`) | Haiku | W2 | M1 | done |
| M3 | Safety exceptions (`safety.py`) | Haiku | W2 | M1 | done |
| M4 | `MAKERExecutor.run_powershell` (subprocess + asyncio + Windows kill chain) | Sonnet | W3 | M2, M3 | done |
| M5 | `PowerShellAdapter` (Tool wrapper around MAKERExecutor) | Sonnet | W4 | M4 | done |
| M6 | Prompt templates + `format_steps` + `goal_achieved` parser | Sonnet | W3 | M2 | done |
| M7 | `IterativeGoalExecutor` — 6-phase loop with `asyncio.gather`, cost tracking, max-iter | Opus | W5 | M4, M5, M6 | done |
| M8 | Unit tests for executor / state / prompts | Sonnet | W4 | M4, M6 | done |
| M9 | Dispatcher wiring + `@goal` parser + `Intent.kind="goal"` | Sonnet | W6 | M5, M7 | done |
| M10 | E2E gate: user goal → 1–3 iterations → goal-achieved → Result with cost & latency | Opus | W7 | M9 | done |
